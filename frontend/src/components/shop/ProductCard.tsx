  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gray-50">
        <img
          src={product.image ?? 'https://images.unsplash.com/photo-1619566636858-adf3ef46400b?w=400&q=80'}
          alt={product.name}
      ...
      {/* Info */}
      <div className="p-3">
        <h3 className={`font-semibold text-gray-800 truncate ${compact ? 'text-sm' : 'text-base'}`}>{product.name}</h3>
        ...
        {/* Unit */}
        {product.unit && (
          <p className="text-xs text-gray-400 mt-0.5">per {product.unit.symbol}</p>
        )}

        {/* Price */}
        <div className="flex items-center justify-between mt-2">
          <div>
            <span className={`font-bold text-themeColor ${compact ? 'text-base' : 'text-lg'}`}>
              ₹{product.price}
            </span>
          ...
          >
            {adding ? <FiStar size={14} className="animate-spin" /> : <FiShoppingCart size={14} />}
          </button>
        </div>
